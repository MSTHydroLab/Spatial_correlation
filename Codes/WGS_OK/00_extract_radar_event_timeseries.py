...
FNAME_RE = re.compile(
    r"Composite_[^_]+_(?P<accum_sec>\d+)_(?P<date>\d{2}[A-Z]{3}\d{4})_(?P<time>\d{6})\.out$",
    re.IGNORECASE,
)
...
def discover_files_in_window(comp_dir: Path, start: pd.Timestamp, end: pd.Timestamp) -> dict[pd.Timestamp, Path]:
    found: dict[pd.Timestamp, Path] = {}
    for p in sorted(comp_dir.glob("*.out")):
        try:
...
def build_event_cube(
    ...
    files = discover_files_in_window(comp_dir, start, end)
    if not files:
        raise FileNotFoundError(f"No radar files found in window for event {event_number} under {comp_dir}")
    ...
    for i, ts in enumerate(event_idx):
        ...
        for a in attrs:
            ...
            if not same:
                raise ValueError(f"Header changed within {comp_dir} for {fp.name}: field {a} differs")
    ...
    meta_df.insert(1, "composite_name", comp_dir.name)
    ...
    missing_df = pd.DataFrame({
        ...
        "reason": "file_not_found_in_composite_folder",
    })

    summary_df = pd.DataFrame([{
        "event": int(event_number),
        "composite_name": comp_dir.name,
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Extract radar rainfall time series at existing WGS grid centers for a given event."
    )
...
def main() -> None:
    ...
    comp_dirs = [Path(p) for p in args.composite_dirs]
    ...
    for comp_dir in comp_dirs:
        if not comp_dir.exists():
            raise FileNotFoundError(f"Composite folder not found: {comp_dir}")
        print(f"\n[composite] processing {comp_dir}")
        build_event_cube(
            event_number=args.event,
            comp_dir=comp_dir,
            out_dir=out_root / comp_dir.name,
            grid_map=grid_sub,
            start=start,
            end=end,
            event_idx=event_idx,
